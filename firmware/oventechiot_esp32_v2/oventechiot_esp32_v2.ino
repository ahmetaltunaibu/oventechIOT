/*
 * OventechIOT — ESP32 Köprü Firmware'i (v2 — HMI KÖPRÜSÜ MODU)
 * ======================================
 * NORMAL (v1.x) firmware'den TEK farkı: RS485 hattının ucunda artık PLC
 * değil, PLC'ye bağlı Delta HMI var (HMI kendi Lua script'iyle gerçek PLC
 * adreslerini kendi mem.inter belleğinde 500-530 register aralığına
 * aynalıyor, ESP32 de o aralığı okuyor/yazıyor). Sunucudaki tag tablosu
 * BİLEREK değiştirilmedi — hâlâ eski (doğrudan PLC) ham adresleri
 * gösteriyor. Bu yüzden burada, sunucudan gelen tag listesi ayrıştırılırken
 * her tag'in eski adresi ADRES_CEVIRI_TABLOSU ile HMI'deki yeni adresine
 * çevriliyor (bkz. adresCevir()) — sunucu ve tag tablosu tamamen olduğu
 * gibi kalıyor, tek değişen bu ESP32'nin fiilen sorduğu adres.
 *
...
// ================== HMI KÖPRÜSÜ: ADRES ÇEVİRİ TABLOSU ==================
// Sol taraf: sunucudaki tag'in ham Modbus adresi (eski, doğrudan PLC).
// Sağ taraf: aynı sinyalin HMI'nin mem.inter'inde aynalandığı register
// (bkz. HMI'ye yüklenen Lua script). Listede olmayan bir adres gelirse
// (yeni eklenen bir tag gibi) OLDUĞU GİBİ bırakılır.
struct AdresCevirisi { uint16_t eski; uint16_t yeni; };
const AdresCevirisi ADRES_CEVIRI_TABLOSU[] = {
  {4128, 500}, {4522, 502}, {4518, 504}, {4520, 506},   // float (2 register)
  {4096, 500}, {4098, 502},                              // ayni tag'in kopya/eski kaydi (id 72/73) icin de gecerli olsun
  {4504, 508}, {4505, 509},                              // int (1 register)
  {1024, 510}, {1025, 511}, {1026, 512}, {1027, 513}, {1028, 514},
  {1029, 515}, {1040, 516}, {1041, 517}, {1042, 518}, {1043, 519},
  {1044, 520},                                            // X - salt okunur bit
  {1280, 521}, {1282, 522}, {1285, 523}, {1296, 524}, {1297, 525},
  {1299, 526}, {1300, 527},                               // Y - oku+yaz bit
  {2048, 528}, {2052, 529}, {2153, 530},                  // M - oku+yaz bit
};
const int ADRES_CEVIRI_SAYISI = sizeof(ADRES_CEVIRI_TABLOSU) / sizeof(ADRES_CEVIRI_TABLOSU[0]);

uint16_t adresCevir(uint16_t eskiAdres) {
  for (int i = 0; i < ADRES_CEVIRI_SAYISI; i++) {
    if (ADRES_CEVIRI_TABLOSU[i].eski == eskiAdres) return ADRES_CEVIRI_TABLOSU[i].yeni;
  }
  return eskiAdres;
}
...
bool tagListesiniGetir() {
  ...
  for (JsonObject t : dizi) {
    ...
    // JsonVariantConst olarak alıp öyle .as<String>() çağırıyoruz.
    {
      ...
      hedef.modbusAdres = adresCevir((uint16_t)modbusAdresStr.toInt());
    }
...
String tagOku(const TagTanimi &tag) {
  ...
  if (tip == "bool") {
    ...
    // HMI KÖPRÜSÜ İSTİSNASI: adres HMI'nin mem.inter aralığına (500-530)
    // çevrilmişse, gerçek PLC'deki erisim ne olursa olsun HOLDING REGISTER
    // (fonksiyon 03) kullan — deneyerek bulundu: HMI'nin Modbus slave'inde
    // COIL (01/05) ve HOLDING REGISTER (03/06) apayrı bellek alanlarına
    // gidiyor; mem.inter.Read/Write (Lua tarafı) SADECE holding-register
    // tarafını görüyor. COIL ile yazınca ESP32 kendi coil-okumasında
    // "başarılı" görse de Lua bunu HİÇ görmüyor, PLC'ye hiç ulaşmıyordu
    // (kullanıcı raporu — LAMBA fiziksel olarak hiç açılmıyordu).
    String erisim = String(tag.erisim); erisim.toLowerCase();
    bool hmiKoprusuAdresi = (tag.modbusAdres >= 500 && tag.modbusAdres <= 530);
    uint8_t sonuc = hmiKoprusuAdresi
      ? modbus.readHoldingRegisters(tag.modbusAdres, 1)
      : ((erisim == "read")
...
bool tagYaz(const TagTanimi &tag, const String &degerStr) {
  ...
  if (tip == "bool") {
    // Okuma tarafındaki HMI KÖPRÜSÜ İSTİSNASI ile aynı sebep: 500-530
    // aralığında holding register (fonksiyon 06) kullan, coil değil.
    bool hmiKoprusuAdresi = (tag.modbusAdres >= 500 && tag.modbusAdres <= 530);
    uint8_t sonuc = hmiKoprusuAdresi
      ? modbus.writeSingleRegister(tag.modbusAdres, degerStr.toInt() != 0 ? 1 : 0)
      : modbus.writeSingleCoil(tag.modbusAdres, degerStr.toInt() != 0);
